...
export default async function AtelierPage() {
  const rows = await prisma.setting.findMany({
    where: { key: { in: ["atelier_overline", "atelier_title", "atelier_body", "atelier_image1", "atelier_image2", "atelier_footer", "atelier_capsule_title", "atelier_capsule_text", "atelier_capsule_image", "atelier_capsule_visible"] } },
  })
  ...
  const capsuleVisible = s.atelier_capsule_visible === "true"
  const capsuleTitle = s.atelier_capsule_title ?? "Capsule"
  const capsuleText  = s.atelier_capsule_text  ?? ""
  const capsuleImage = s.atelier_capsule_image ?? ""
